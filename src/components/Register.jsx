...
const Register = () => {
    ...
    const inputClasses = "w-full h-[48px] pl-12 pr-4 border-2 border-input rounded-[12px] bg-muted/50 text-[15px] text-foreground outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition-all placeholder-muted-foreground/70 font-medium";
    const labelClasses = "block text-[14px] font-bold text-foreground mb-2";

    return (
        <div className="auth-wrapper relative min-h-[calc(100vh-80px)] w-full overflow-x-hidden flex justify-center items-center bg-background font-['Inter',_sans-serif] py-20">
             <style>{`
            ...
            <div className={`animated-background fixed top-0 left-0 w-full h-full z-0 pointer-events-none transition-all duration-400 opacity-60 ${isFocused ? 'blurred' : ''}`}>
                {backgroundCards.map((card) => (
                    <div key={card.id} className="floating-lane-container absolute will-change-transform"
                        ...
                        <div className="bg-card w-[360px] rounded-[20px] shadow-lg overflow-hidden flex flex-col border border-border opacity-85 text-left rotate-[-12deg]">
                            <div className="relative w-full h-[200px] bg-muted">
                                ...
                                <div className="absolute top-4 right-4 bg-card text-foreground font-bold text-[11px] px-3 py-1 rounded-md shadow-sm">{card.event.date}</div>
                            </div>
                            <div className="p-6 bg-card">
                                <h3 className="text-[18px] font-bold text-foreground mb-2 truncate">{card.event.title}</h3>
                                <p className="text-[14px] text-muted-foreground mb-4 line-clamp-2">{card.event.desc}</p>
                                <div className="flex justify-between items-center text-[13px] text-muted-foreground font-medium">
                                    <div className="flex items-center gap-1.5"><MapPin size={12} className="text-muted-foreground" /> {card.event.location}</div>
                                    <button className="bg-muted text-foreground px-4 py-2 rounded-lg border border-border">Detalii</button>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div className="auth-container relative z-10 w-full max-w-[500px] p-4" onMouseEnter={() => setIsFocused(true)} onMouseLeave={() => setIsFocused(false)}>
                <div className="auth-card bg-card rounded-[30px] shadow-2xl dark:shadow-black/40 border-2 border-border p-10">
                    <div className="mb-8 text-left">
                        <h2 className="text-[36px] font-extrabold text-foreground mb-2 tracking-tight font-light">Creează Cont</h2>
                        <p className="text-[16px] text-muted-foreground font-medium">Alătură-te Event Manager astăzi.</p>
                    </div>

                    <form onSubmit={handleSubmit} className="space-y-5">
                        <div><label className={labelClasses}>Prenume</label><div className="relative"><User className="absolute left-4 top-1/2 -translate-y-1/2 text-muted-foreground z-10" size={18} /><input type="text" name="firstName" placeholder="Ion" value={formData.firstName} onChange={handleChange} required className={inputClasses} /></div></div>
                        <div><label className={labelClasses}>Nume</label><div className="relative"><User className="absolute left-4 top-1/2 -translate-y-1/2 text-muted-foreground z-10" size={18} /><input type="text" name="lastName" placeholder="Popescu" value={formData.lastName} onChange={handleChange} required className={inputClasses} /></div></div>
                        <div><label className={labelClasses}>Telefon</label><div className="relative"><Phone className="absolute left-4 top-1/2 -translate-y-1/2 text-muted-foreground z-10" size={18} /><input type="tel" name="phoneNumber" placeholder="07xxxxxxxx" value={formData.phoneNumber} onChange={handleChange} required className={inputClasses} /></div></div>
                        <div><label className={labelClasses}>Email</label><div className="relative"><Mail className="absolute left-4 top-1/2 -translate-y-1/2 text-muted-foreground z-10" size={18} /><input type="email" name="email" placeholder="student@university.edu" value={formData.email} onChange={handleChange} required className={inputClasses} /></div></div>
                        
                        <div className="space-y-5">
                            <div><label className={labelClasses}>Facultate</label><div className="relative"><Building2 className="absolute left-4 top-1/2 -translate-y-1/2 text-muted-foreground z-10" size={18} /><select name="faculty" value={formData.faculty} onChange={handleChange} required className={inputClasses + " appearance-none"}>
                            ...
                            <div><label className={labelClasses}>An Studiu</label><div className="relative"><Timer className="absolute left-4 top-1/2 -translate-y-1/2 text-muted-foreground z-10" size={18} /><select name="studentYear" value={formData.studentYear} onChange={handleChange} required className={inputClasses + " appearance-none"}><option value="">Selectează</option><option value="1">Anul 1</option><option value="2">Anul 2</option><option value="3">Anul 3</option><option value="4">Anul 4</option></select></div></div>
                        </div>

                        <div><label className={labelClasses}>Parolă</label><div className="relative"><Lock className="absolute left-4 top-1/2 -translate-y-1/2 text-muted-foreground z-10" size={18} /><input type="password" name="password" placeholder="••••••••" value={formData.password} onChange={handleChange} required className={inputClasses} /></div></div>
                        <div><label className={labelClasses}>Confirmă</label><div className="relative"><Lock className="absolute left-4 top-1/2 -translate-y-1/2 text-muted-foreground z-10" size={18} /><input type="password" name="confirmPassword" placeholder="••••••••" value={formData.confirmPassword} onChange={handleChange} required className={inputClasses} /></div></div>
                    ...
                    <div className="mt-8 text-center text-[14px] text-muted-foreground font-medium">Ai deja un cont? <Link to="/login" className="text-[#ff7675] font-bold hover:underline">Autentifică-te</Link></div>
                </div>
            </div>
        </div>
    );
